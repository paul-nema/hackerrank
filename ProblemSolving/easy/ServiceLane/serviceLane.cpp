...
//  cases: a two dimensional array of integers where each element
//  is an array of two integers representing starting and ending indices for a segment to consider .
//  width: array of lanes
vector<int> serviceLane( int n, vector< vector< int > > &cases, vector< int > &width ) {
    vector< int > largestSegment;

    for( auto x: cases ) {
        largestSegment.push_back( *std::min_element( width.begin() + x[ 0 ], width.begin() + x[ 1 ] + 1 ) );
    }
